initial_team_selection_roi <- function(df){
  ...
  # pick 3 players with highest points from last season
  top_players <-
    df %>% 
  ...
  print(top_players %>% count(position))
  
  mid_count <- 2
  fwd_count <- 1
  
  # print(df %>% anti_join(top_players) %>% arrange(desc(total_points)))
  ...
  model <- MIPModel() %>%
    ...
    add_constraint(sum_expr(x[i], i = which(filtered_data$position == "MID")) == 3) %>%
