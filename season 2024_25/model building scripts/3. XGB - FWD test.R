...
input_df <-
  input_df %>%
  filter(total_points >= 3 & total_points <= 6)
...
bind_cols(train_df %>% select(total_points),
          predict(wf_fitted,train_df)) %>%
  mutate(error = total_points-.pred) %>% 
  ggplot(aes(x=total_points,y=error))+
  geom_point()

augment(wf_fitted,train_df) %>%
  select(.pred,total_points) %>% 
  View()
...
bind_cols(test_df %>% select(total_points),
          predict(wf_fitted,test_df)) %>%
  mutate(error = abs(total_points-.pred)) %>% 
  ggplot(aes(x=(total_points),y=error))+
  geom_point()
...
# 
# model_metrics %>% 
#   write_csv(here(paste0("models/season ",curr_season),
#                  "XGBMod_FWD_metrics_test.csv"))
# 
# # Save Model ----
# wf_fitted %>%
#   readr::write_rds(here(paste0("models/season ",curr_season),
#                         "XGBMod_FWD_Reg_test.rds"))