# Import Libraries ----
library(tidyverse)
library(here)

# Import Helper Functions ----
source(here("scripts/season 2023_24","99. Constants.R"))
source(here("utils/import_all_gws.R"))
source(here("utils/cast_correct_dtypes.R"))
source(here("utils/add_opponent_team_name.R"))
source(here("utils/add_season.R"))
source(here("utils/preprocessing.R"))
source(here("utils/fpl_lag_data.R"))
source(here("utils/fpl_sum_all_previous.R"))
source(here("utils/fpl_sum_n_previous.R"))
source(here("utils/fpl_avg_n_previous.R"))
source(here("utils/import_all_players_understat.R"))
source(here("utils/generate_seasons.R"))
source(here("utils","row_bind_pattern_objects.R"))

# Import Data ----
# **** Historical GoalWeeks ----
seasons <- generate_seasons(first_season, curr_season)

for (season in seasons[1:length(seasons)-1]){
  print(season)
  
  var_name <- paste0("season_", season, "_gws")
  
  assign(var_name, import_all_gws(paste0("data/raw/",
                                         str_replace(season,coll("_"),"-"),"/gws"),
                                  season))
}

season_combined_gws <- row_bind_pattern_objects("_gws")

# **** Team ID and Name data ----
# Include Historical and Current Season
for (season in seasons){
  print(season)
  
  var_name <- paste0("season_", season, "_teams")
  
  assign(var_name, 
         read_csv(here(paste0("data/raw/",str_replace(season,coll("_"),"-")),
                       "teams.csv")
                            ) %>%
           mutate(season_x = season)
           
         )
}

season_combined_teams <- row_bind_pattern_objects("_teams")

# **** Team Strength Data ----
for (season in seasons[1:length(seasons)-1]){
  print(season)
  
  var_name <- paste0("season_", season, "_fdr")
  
  assign(var_name, 
         read_csv(here(paste0("data/raw/",str_replace(season,coll("_"),"-")),
                       "fixtures.csv")) %>%
           mutate(season_x = season)
         )
}

season_combined_fdr <- row_bind_pattern_objects("_fdr")

# **** Player Understat Data ----
# Historical and Current Seasons
for (season in seasons){
  print(season)
  
  var_name <- paste0("season_", season, "_players_understat")
  
  assign(var_name, 
         import_all_players_understat(paste0("data/raw/",
                                             str_replace(season,coll("_"),"-"),"/understat")))
}

season_combined_understat <- row_bind_pattern_objects("_understat")

season_combined_understat <-
  season_combined_understat %>% 
  distinct()

# Export ----
season_combined_gws %>% 
  write_csv(here(paste0("data/processed/historical/season ",curr_season),
                 "season_combined_gws.csv"))

season_combined_understat %>% 
  write_csv(here(paste0("data/processed/player_understat/season ",curr_season),
                 "season_combined_players_understat.csv"))

season_combined_fdr %>% 
  write_csv(here(paste0("data/processed/fdr/season ",curr_season),
                 "season_combined_fdr.csv"))

season_combined_teams %>% 
  write_csv(here(paste0("data/processed/team_id_name/season ",curr_season),
                 "season_combined_teams.csv"))