...
# upcoming fdr ----
upcoming_fdr <-
  read_csv(here("data/processed/fdr","upcoming_season.csv"))
...
current_season_fdr_df <- read_csv(here("data/raw/2023-24","fixtures.csv"))
...
historical_df <- import_historical_data("data/processed/historical")
historical_df <-
  historical_df %>% 
  filter(!is.na(name))
...
single_gw_df <- 
  import_all_gws("data/processed/upcoming")
